...
function Result() {
    ...
    const totalCreditsEarned = grades.filter(g => g.passed).reduce((sum, g) => sum + g.courseId?.credits, 0);
    ...
    filteredGrades.forEach(g => {
        if (g.score !== undefined) {
            ...
            semGradePoints += (getGradePoint(g.grade) * credits);
        } else {
            allGraded = false;
        }
    });

    const sgpa = semCreditsAttempted > 0 ? (semGradePoints / semCreditsAttempted).toFixed(2) : "0.00";

    return (
        <main className="flex flex-col min-h-[calc(100vh-80px)] bg-[#f4f7f6]">
            <div className="flex-grow px-10 py-10 max-w-[1200px] mx-auto w-full">
                
                <div className="flex justify-between items-end mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-[#510443] mb-2">Academic Transcript</h1>
                        <p className="text-gray-500 font-medium tracking-wide">View your official grades and performance.</p>
                    </div>
                        ...
                        <div className="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
                            <div className="bg-[#510443] px-6 py-4 flex justify-between items-center text-white">
                                ...
                                {allGraded ? (
                                    ...
                                    <span className="font-bold text-sm bg-yellow-500/20 text-yellow-300 px-3 py-1 rounded-full">
                                        Semester Ongoing
                                    </span>
                                )}
                            </div>
